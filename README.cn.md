# dealve-tui

[English version](./README.md)

Delve into game deals from your terminal 👾

[![x-cmd/install — dealve-tui Code Quality Monitoring Repo Card](https://x-cmd.com/repo-card/dealve-tui.svg?lang=zh)](https://x-cmd.com/install/dealve-tui)

## 安装

```sh
x install dealve-tui
```

## 代码洞察

合计: **5,423** 行代码（覆盖前 5 种语言、共 **28** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Rust | 5,342 | 165 | 598 | 22 |
| Toml | 81 | 0 | 7 | 4 |
| Markdown | 0 | 83 | 45 | 2 |

## 源代码

- **上游仓库**: <https://github.com/kurama/dealve-tui>
- **官网**: <https://crates.io/crates/dealve-tui>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `v1.0.2` (2026-02-23)
- **最近提交**: 2026-05-08
- **Release 含资产**: 5 个

## 流行度

- **Star**: 245 · **Fork**: 8 · **开放 issue**: 13 · **贡献者**: 3

## 累计统计

- **发布数**: 3 · **已合并 PR**: 12 · **开放 PR**: 2 · **已关闭 issue**: 10 · **开放 issue**: 3 · **提交数**: 83

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-26 | 0 | 0 | 0 | 0 | 0 | 0 |
| last60d | 2026-07-27 | 0 | 0 | 0 | 0 | 0 | 0 |
| 90d | 2026-06-27 | 0 | 0 | 0 | 0 | 0 | 0 |
| last180d | 2026-03-29 | 0 | 1 | 2 | 0 | 1 | 2 |
| 360d | 2025-09-30 | 3 | 12 | 2 | 10 | 3 | 83 |
| last720d | 2024-10-05 | 3 | 12 | 2 | 10 | 3 | 83 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [dealve-linux-aarch64](https://github.com/kurama/dealve-tui/releases/download/v1.0.2/dealve-linux-aarch64) | 2.4 MiB | `native/linux/arm64` |
| [dealve-linux-x86_64](https://github.com/kurama/dealve-tui/releases/download/v1.0.2/dealve-linux-x86_64) | 2.5 MiB | `native/linux/x64` |
| [dealve-macos-aarch64](https://github.com/kurama/dealve-tui/releases/download/v1.0.2/dealve-macos-aarch64) | 2.3 MiB | `native/darwin/arm64` |
| [dealve-macos-x86_64](https://github.com/kurama/dealve-tui/releases/download/v1.0.2/dealve-macos-x86_64) | 2.3 MiB | `native/darwin/x64` |
| [dealve-windows-x86_64.exe](https://github.com/kurama/dealve-tui/releases/download/v1.0.2/dealve-windows-x86_64.exe) | 2.7 MiB | `native/win/x64` |

## 改进这些数据

dealve-tui 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `dealve-tui` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/dealve-tui.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260925.yml` · 2026-09-25T04:45:22Z._
